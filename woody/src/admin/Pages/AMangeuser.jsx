...
function AMangeuser() {
    ...
    const statushandle = async(id)=>{
        ...
        const cuurntstatus = res.data.status
        try {
          if(cuurntstatus === "block")
            {
                const res = await axios.patch(`http://localhost:3000/user/${id}`,{status:"unblock"})
                {
                        ...
                        
                    {
                        ...
                        toast.success("Status chnaged to unblock successfull")
                        fetch()
                        
                    }
                }
            }
            else if(cuurntstatus === "unblock")
                {
                    const res = await axios.patch(`http://localhost:3000/user/${id}`,{status:"block"})
                    {
                        if(res.status === 200)
                        {
                            toast.success("Status chnaged to block successfull")
                            fetch()
                        }
                    }
                }
